...
const Header = () => {
  ...
  const auth = useAuth();
  const [openLoginPrompt, setOpenLoginPrompt] = useReactState(false);
    ...
    <>
      ...
      <Dialog open={openLoginPrompt} onOpenChange={setOpenLoginPrompt}>
        <DialogTrigger asChild>
          {/* Hidden: only open via setOpenLoginPrompt */}
        ...
        <DialogContent className="max-w-xs mx-auto text-center">
          <div className="space-y-4">
            ...
            <LoginDialog />
            <Button variant="ghost" onClick={() => setOpenLoginPrompt(false)}>Close</Button>
          </div>
        </DialogContent>
      </Dialog>
